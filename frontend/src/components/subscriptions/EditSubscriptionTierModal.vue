...
const handleSubmit = async () => {
  ...
  try {
    const result = await subscriptionStore.updateTier(props.tier.id, transformedData);
    if (result.success) {
      console.log('Tier updated successfully:', result.tier);
      emit('update', result.tier);
      emit('close');
    } else {
