...
const emit = defineEmits(['dragStart', 'drop', 'dragMove', 'settings', 'addFreePreview']);
...
const handleRemove = () => {
  ...
  uploadStore.removePreview(props.preview.id);
  isSettingsOpen.value = false;
};
...
watch(isEditorOpen, (newValue) => {
  if (newValue && isImage.value) {
    console.log('ImageEditor opened with URL:', currentEditingVersion.value?.url || props.preview.url);
  }
});

watch(() => props.preview.previewVersions, (newVersions) => {
  if (newVersions && newVersions.length > 0) {
    const latestVersion = newVersions[newVersions.length - 1];
    if (isImage.value) {
      currentEditingVersion.value = latestVersion;
      isEditorOpen.value = true;
    }
  }
}, { immediate: true });
