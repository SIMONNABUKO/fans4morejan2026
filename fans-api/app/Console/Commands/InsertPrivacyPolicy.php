...
class InsertPrivacyPolicy extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'privacy:insert';
     ...
     * @var string
     */
    protected $description = 'Insert privacy policy data into the database';
    ...
    {
        $this->info('Inserting privacy policy...');

        // Check if privacy policy already exists
        if (PrivacyPolicy::count() > 0) {
            $this->warn('Privacy policy already exists in the database.');
            return;
        }
...
HTML;
        ...
        try {
            ...
            $this->info('Privacy policy inserted successfully!');
        } catch (\Exception $e) {
            $this->error('Failed to insert privacy policy: ' . $e->getMessage());
        }
    }
}
